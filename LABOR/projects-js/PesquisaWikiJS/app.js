const url =
  'https://br.wikipedia.org/w/api.php?action=query&list=search&srlimit=20&format=json&origin=*&srsearch=';
...
const renderResults = (list) => {
  const cardsList = list
    .map((item) => {
      ...
      return `<a href=http://br.wikipedia.org/?curid=${pageid} target="_blank">
            <h4>${title}</h4>
            <p>
              ${snippet}
            </p>
